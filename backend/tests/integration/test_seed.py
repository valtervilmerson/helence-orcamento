...
from app.db.seed import SEED_PRICE_TABLE_CODE, SEED_USERS, seed
...
def test_seed_creates_vigente_price_table(connection) -> None:
    seed(connection)

    row = connection.execute(
        "SELECT status FROM price_tables WHERE code = ?", (SEED_PRICE_TABLE_CODE,)
    ).fetchone()
...
def test_seed_is_idempotent(connection) -> None:
    ...
    price_table_count = connection.execute(
        "SELECT COUNT(*) AS n FROM price_tables WHERE code = ?", (SEED_PRICE_TABLE_CODE,)
    ).fetchone()["n"]

    assert user_count == len(SEED_USERS)
    assert price_table_count == 1
