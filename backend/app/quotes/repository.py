    ...
    *,
    ...
    cursor = connection.execute(
        ...
        INSERT INTO quotes (
            ...
            installment_count, installment_interest_percent, entrada_amount, entrada_percent
        )
        VALUES (?, ?, 'rascunho', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
...
_QUOTE_BASE = """
    SELECT
        ...
        q.entrada_percent AS entrada_percent,
        c.id AS customer_id,
...
def get_item_components(connection: sqlite3.Connection, item_id: int) -> list[sqlite3.Row]:
    return connection.execute(
        ...
        SELECT qic.id AS id, qic.component_variant_id AS component_variant_id,
               ...
               cv.dimension_id AS dimension_id, d.raw_label AS dimension_label
        FROM quote_item_components qic
        LEFT JOIN skus s ON s.id = qic.sku_id
