...
def is_pure_dimension_text(text: str) -> bool:
    """Returns True when text contains only dimension tokens (digits, x, H, L, P, D, M, spaces)."""
    return not re.sub(r"[0-9xXhHlLpPdDmM\s]", "", text)
...
def build_item(
    *,
    ...
    return {
        ...
        "finish": finish,
        "finish_group": finish_group if finish_group is not None else finish_group_for(finish),
        "sku": sku,
        ...
        for col in sku_cols:
            ...
            for previous_idx in reversed(nonempty_idx[max(0, pos - 8) : pos]):
                ...
                if not candidate or candidate.upper().startswith("TOTAL"):
                    continue
                candidate_dim = normalize_dimension(candidate)
                # Use as description if: (a) not a dimension at all, or
                # (b) normalizes to a dimension but also contains description words
                # (e.g. "NUVEM CIRCULAR D600" → dim="D600" but has description content)
                if not description and (not candidate_dim or not is_pure_dimension_text(candidate)):
