"""Spike: apenas o recorte do grupo hibrido 'TAMPOS / ESTRUTURAS / APOIO' (sheet5/raw_rows
tem token XML invalido e ja foi tratado em 08; aqui evitamos reler aquela aba)."""
import zipfile
...
for row in gen:
    if row[idx["grupo"]] == "TAMPOS / ESTRUTURAS / APOIO":
        ...
        paginas_hibrido.add(int(row[idx["pagina_pdf"]]))
        if shown < 6:
            print(" ", row)
            shown += 1

print(f"\nPaginas com grupo hibrido (todas): {sorted(paginas_hibrido)}")
